	...
	$('.retail-form-cta').on('click', function() {
		...
		if(mobile.length > 0){
			if(mobile.trim() == ''){
				...
				$('#mobile').parent().addClass('error');
				error = true;
				smoke.alert("Entered mobile number is Incorrect . Kindly check and enter the correct number");
			}
		    ...
		    $.ajax({
		    	...
		    	//error: function(jqXHR, textStatus, error){
                    ...
                    //if (jqXHR.responseJSON.errors.mobile){
						//smoke.alert("Request already registered under this mobile number");                        
						//$('#mobile').parent().addClass('error');
