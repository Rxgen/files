...
.addressLink address .cin-no,.addressLink address .telephone{
    margin-bottom: 0.781vw;
    font-size: 0.938vw;
}

.addressLink address .telephone a{
    color: rgba(#fff, .8);
    font-size: 0.938vw;
}
...
@media (max-width: 600px) {
    ...
    .addressLink address .cin-no,.addressLink address .telephone, .addressLink address .telephone a{
        font-size: 3.865vw;
        margin-bottom: 3.623vw;
    }
}
...
<footer class="footerContainer">
    <div class="footerSocialIconsContainer">
        <a href="{{setting('site.facebook_social_media_icon_mp')}}" target="_blank" rel="noopener noreferrer" class="socialIconBox">
            <i class="fa fa-facebook" aria-hidden="true"></i>
        </a>
        <a href="{{setting('site.linkedin_social_media_icon_mp')}}" target="_blank" rel="noopener noreferrer" class="socialIconBox">
            <i class="fa fa-linkedin" aria-hidden="true"></i>
        </a>
        <a href="{{setting('site.youtube_social_media_icon_mp')}}" target="_blank" rel="noopener noreferrer" class="socialIconBox">
    ...
    <address>
            <h4>Nayara Energy Limited</h4>
            <p class="address"><span>Registered Office address</span>: Khambhalia Post, P O Box 24, District Devbhumi <br>Dwarka - 361 305, Gujarat, India</p>
...
{{--@if ($currentroute === 'Newhome.homepage')
<a href="" class="linkBtn"></a>
<div class="popUp-alert">
    <h3>!! Kind Attention All Ex-employees !! <br>Important message related to PF</h3>
    <div class="popUp_container">
        ...
        <p>While reviewing the PF Trust performance, it was observed that many of the employees have
        left the Company, but their PF is not yet settled and the balance is still lying with the Nayara PF
        Trust in the unclaimed balances. The EPFO Office also has informed to reach out to the
        ex-employees to get their PF account settled.</p>
        <p>In view of that, we are reaching out to all the ex-employees, so that they can directly contact
        the PF Trust team and arrange to get their PF Settled.</p>
        <p>You are requested to immediately reach out to Mr. Jignesh Darji on
        <a href="mailto:Jignesh.darji@nayaraenergy.com">Jignesh.darji@nayaraenergy.com</a> or through mobile at <a href="tel:+9004476318">9004476318.</a></p>
    </div>
    <div class="close-popup"><img src="{{asset('images/cancel.png')}}" alt="Cancel" width="32" height="32"></div>
</div>
@endif--}}
...
<!-- For Hiding  Popup's on every page - below code is commetnted  -->
...
@if ( (Request::segment(1) == 'retail') || (collect(request()->segments())->last() === ('retail-footprint')) || (collect(request()->segments())->last() === ('franchisees-eligibility')) || (Request::segment(1)=== ('career-opening')) || (collect(request()->segments())->last() === ('sustainable-development')) /* || \Route::current()->getName() == 'home.homepage'*/) 
    ...
    @endif
	<div class="popUp2" id="notice_modal" style = {{ collect(request()->segments())->last() === ('apply-online') ? 'display:none!important' : ' ' }}>
        ...
        <div class="popUp2__content">
            <h4>FRAUD ALERT</h4>
            <h5>Attention</h5>
            <p>Stay alert for fraudulent activities targeting your accounts. Safeguard your information and report any suspicious activity immediately.</p>
            <a href="{{ route('static.contact',['slug'=>'fraud-alert']) }}">Know More</a>
            <img src="{{URL::to('/') }}/images/alert.png" alt="" class="alert-img">
        </div>
    </div>
