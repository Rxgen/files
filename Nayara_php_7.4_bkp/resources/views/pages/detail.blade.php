...
<section class="innerPage-banner-container">
    <picture>
        <source media="(max-width: 767px)" srcset="{{Voyager::image($post->blog_mobile_image)}}" />
    <img src="{{Voyager::image($post->blog_desktop_image)}}" alt="{{$post->title}}" title="{{$post->title}}"class="innerBanner-img">
    </picture>
</section>
@include('includes.bread-crumbs')

<section class="blog_wrapper">
<h1 class="innerPageTitle theme-gradient-alpha gradient-text">{{$post->title}}</h1>
            ...
            <section class="investorPages-container">
                <div class="innerPage-content innerMargin">
                    ...
                    @foreach($blogrelatedposts as $blogrelatedpost)
                        ...
                        <div class="gridImage"><img src="{{Voyager::image($blogrelatedpost->Image)}}" alt=" {{$blogrelatedpost->title}} " class=""></div>
