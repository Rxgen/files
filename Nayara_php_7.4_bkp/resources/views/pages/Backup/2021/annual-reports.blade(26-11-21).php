...
<div class="search-block">
    <img src="{{asset('images/investors/search-icon.png')}}" alt="search">
...
<h2 class="annual-reportSubsidiaries-title">Annual Reports</h2>
    ...
    <div class="pdfDownload-data">
        ...
        <div class="pdfdata-block">
            <div class="pdf-name-title">
               Annual Report FY ( {{ $report->year }} - {{ $report->year+1 }} )
            </div>
...
/*<div class="search-block">
    <img src="{{asset('images/investors/search-icon.png')}}" alt="search">
    <select id="year-change-quarterly" class="yearSelection-annualReport">
        <option value="">Select Year</option>
        @if(!empty($quarterly_results_years[0]->max_year) && !empty($quarterly_results_years[0]->min_year))
        @for($i=$quarterly_results_years[0]->max_year; $i>=$quarterly_results_years[0]->min_year; $i--)
        <option value="{{$i}}">{{$i}}-{{(int)$i+1}}</option>
        @endfor
        @endif
    </select>
    <input type="hidden" class="ajax-url-quarterly" value="{{route('ajax.quarterlyresult')}}">
</div>
<h2 class="annual-reportSubsidiaries-title">Quarterly Financial Results</h2>
<div id="report-div-result" class="all-reports-listingBlock annual-report-listing wrap-clear">
    @if($quarterly_results->isNotEmpty())
