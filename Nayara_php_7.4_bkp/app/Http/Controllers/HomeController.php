...
class HomeController extends Controller
{
    public function index(){
    	...
    	//dd($blogs);
        $front_news = News::select('news_image','news_title','news_date','news_source','img_title','img_alt','news_url')->where('on_front',1)->where('status',1)->orderBy('created_at','DESC')->get();
    	$slider_news = News::select('news_image','news_title','news_date','news_source','news_url')->where('on_front',0)->where('status',1)->orderBy('created_at','DESC')->get();
    	...
    	//dd($home_images);
		$top_banners = HomePageTopBanner::where('status', 1)->orderBy('order', 'ASC')->get();

        /*check device*/
        $isMob = is_numeric(strpos(strtolower($_SERVER['HTTP_USER_AGENT']), "mobile"));
    	//dd($top_banners);
        //$vi = json_decode($top_banners[1]->video);
    	return view('pages.home')->with(compact(['blogs','front_news','slider_news','products','home_blocks','home_images','top_banners', 'isMob']));
    }
