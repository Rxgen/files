...
class StaticPageController extends Controller
{
	...
	public function applyOnlineFormSubmit(Request $request){
		...
		if($online_lead){
			return response()->json(['RegCode' => $results[0]['RegCode'], 'status' => 1]);
		}
		else{
    ...
    public function getRo(Request $request) {
    	...
    	foreach ($pumps as $key1 => $value1) {
    		$price = $db_ext->table('essar_outlet_product_price')->where('outlet_cmscode', $value1['cms_code'])->get();
    		$fuel_type = $price[0]->fuel_type;
    		$price1 = $price[0]->price;
    		$pumps[$key1]['fuel_type'] = $fuel_type;
    		$pumps[$key1]['price'] = $price1;
    	}
    	//dd($pumps);	
    	return $pumps;
    }
    ...
    public function getCodeRo(Request $request) {
    	...
    	$price = $db_ext->table('essar_outlet_product_price')->where('outlet_cmscode', $request->ro_code)->get();
    	$fuel_type = $price[0]->fuel_type;
		$price1 = $price[0]->price;
		$pumps[0]['fuel_type'] = $fuel_type;
		$pumps[0]['price'] = $price1;
		return $pumps;
